...
def fetch_feed_items(feed_url: str, max_items: int = MAX_FEED_ITEMS) -> list[dict]:
    ...
    try:
        # Fetch with an explicit timeout so one hung feed can't stall the run,
        # then hand the bytes to feedparser (feedparser alone has no timeout).
        resp = requests.get(
            feed_url, timeout=10,
...
def _fetch_feeds_concurrent(urls: list[str], max_items: int = MAX_FEED_ITEMS) -> dict:
    """
    Fetch many feeds in parallel. Returns {url: [items]} — order-independent
    fetch, but callers iterate their url list in order so output stays stable.
    """
    results = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=12) as ex:
        futures = {ex.submit(fetch_feed_items, u, max_items): u for u in urls}
...
def fetch_category_items(feeds: list[str]) -> list[dict]:
    """Aggregate items from multiple feeds (fetched in parallel), deduplicating by title."""
    fetched = _fetch_feeds_concurrent(feeds)
    all_items = []
    seen_titles = set()
    for url in feeds:
        print(f"  → {url}  ({len(fetched.get(url, []))} items)")
...
def build_prompt(category_name: str, items: list[dict], top_n: int) -> str:
    stories_text = "\n\n".join(
        f"[{i+1}] {it['title']}\nSource: {it['source']}\n{it['summary']}"
        for i, it in enumerate(items[:30])   # cap at 30 to stay within token budget
    )
...
# ─────────────────────────────────────────────
# WALLUMBILLA GSH PRICE + GAS PRICE HISTORY  (D5)
# ─────────────────────────────────────────────

GAS_HISTORY_FILE = "gas_history.json"
BRIEFING_SITE_URL = os.environ.get(
    "BRIEFING_SITE_URL",
    "https://dmcalpine76-create.github.io/morning-briefing",
).rstrip("/")
...
def load_gas_history() -> dict:
    """
    Load the rolling gas price history. In CI the workspace is fresh every
    run, so yesterday's history is fetched back from the published site;
    locally a cached copy is used as fallback.
    """
    try:
        r = requests.get(f"{BRIEFING_SITE_URL}/{GAS_HISTORY_FILE}", timeout=10)
        if r.ok:
            data = r.json()
        ...
        pass
    local = Path(__file__).parent / GAS_HISTORY_FILE
    if local.exists():
        try:
...
def update_gas_history(history: dict, au: dict, hh: dict, gsh: dict,
                       out_dir: Path) -> dict:
    """Append today's prices (deduped by date), cap at 30 days, write to output."""
    today = datetime.date.today().isoformat()
    ...
    history = {"days": days, "updated": today}
    try:
        payload = json.dumps(history, indent=1)
        (out_dir / GAS_HISTORY_FILE).write_text(payload, encoding="utf-8")
        (Path(__file__).parent / GAS_HISTORY_FILE).write_text(payload, encoding="utf-8")
    except Exception as e:
        print(f"   ⚠️  Could not write gas history: {e}")
    return history


def _sparkline_svg(values: list, width: int = 84, height: int = 20,
                   color: str = "#c0392b") -> str:
    """Tiny inline SVG sparkline for the gas bar. Returns '' if <2 points."""
...
def gas_price_html(au_gas: dict, hh_gas: dict = None, gsh_gas: dict = None,
                   gas_history: dict = None) -> str:
    """
    ...
    gsh = gsh_gas or {}
    if gsh.get("price") is not None:
        slots += _slot("Wallumbilla", gsh["price"], "$/GJ")
    ...
    # 7-day trend sparkline from history (AU east coast series)
    hist_days = (gas_history or {}).get("days", [])[-7:]
    spark = _sparkline_svg([d.get("au") for d in hist_days])
    if spark:
        slots += f'''<div class="wx-slot wx-gas-slot" title="AU east coast $/GJ — last {len(hist_days)} days">
            <span class="wx-day">7-day</span>
            {spark}
            <span class="wx-rain">trend</span>
        </div>'''
...
def weather_bar_html(brisbane: list[dict], sunshine: list[dict] = None,
                     gas: dict = None, hh_gas: dict = None,
                     gsh_gas: dict = None, gas_history: dict = None) -> str:
    """
    ...
    gas_block       = gas_price_html(gas or {}, hh_gas or {}, gsh_gas or {}, gas_history or {})
...
def generate_html(sections: dict, generated_at: datetime.datetime,
    ...
    token_badge   = ""
    if token_days_left is not None and token_days_left <= 15:
        _tb_col = "#c0392b" if token_days_left <= 5 else "#d97706"
        _tb_n   = max(token_days_left, 0)
        token_badge = (
            f'<span style="font-size:0.6rem;font-weight:700;color:#fff;'
            f'background:{_tb_col};padding:0.12rem 0.45rem;border-radius:3px;'
            f'letter-spacing:0.05em;white-space:nowrap">'
            f'🔑 Outlook token: {_tb_n} day{"s" if _tb_n != 1 else ""} left — '
            f'run: py outlook_email.py setup</span>'
        )
    columns       = "".join(
    ...
    weather_html   = weather_bar_html(weather_data or [], sunshine_data or [], gas_data or {}, hh_gas_data or {},
                                      gsh_gas_data or {}, gas_history or {})
...
<div id="stale-banner" style="display:none;background:#c0392b;color:#fff;text-align:center;
    padding:0.45rem 1rem;font-size:0.78rem;font-weight:700;letter-spacing:0.03em">
...
<script>
// ── Staleness check (D2) — warn if this page isn't today's edition ──
const BRIEFING_DATE_ISO = "{generated_at.strftime('%Y-%m-%d')}";
(function () {{
    const now = new Date();
    const local = now.getFullYear() + '-' +
        String(now.getMonth() + 1).padStart(2, '0') + '-' +
        String(now.getDate()).padStart(2, '0');
    if (local !== BRIEFING_DATE_ISO) {{
...
def item_matches_topic(item: dict, keywords: list[str]) -> bool:
    """
    Return True if the item title or summary matches any keyword.
    Single-word keywords match on word boundaries only, so short terms like
    "AI" no longer match rain/email/Spain. Multi-word phrases use substring
    matching as before.
    """
...
def prefetch_topic_feeds() -> dict:
    """
    Fetch every TOPIC_SEARCH_FEED exactly once, in parallel.
    Previously each topic re-downloaded all ~40 feeds (5 topics x 40 =
    ~200 sequential fetches); now it's one concurrent pass shared by all.
    """
    print(f"   Prefetching {len(TOPIC_SEARCH_FEEDS)} topic feeds (concurrent)…")
    return _fetch_feeds_concurrent(TOPIC_SEARCH_FEEDS, max_items=30)


def fetch_topic_items(keywords: list[str], feed_cache: dict = None) -> list[dict]:
    """Filter the prefetched feed cache to items matching the topic keywords."""
    if feed_cache is None:
        feed_cache = prefetch_topic_feeds()
    all_items  = []
    seen       = set()
    for url in TOPIC_SEARCH_FEEDS:
        for item in feed_cache.get(url, []):
            key = item["title"].lower()[:60]
...
def _token_days_left() -> int | None:
    """
    Days remaining in the 90-day Outlook refresh-token window.
    Reads OUTLOOK_TOKEN_SETUP_DATE (set as a GitHub secret by upload_tokens.py)
    or the local .outlook_token_setup file written by outlook_email.py setup.
    Returns None if the setup date is unknown.
    """
    setup = os.environ.get("OUTLOOK_TOKEN_SETUP_DATE", "").strip()
    if not setup:
        f = Path(__file__).parent / ".outlook_token_setup"
        if f.exists():
            try:
                setup = f.read_text(encoding="utf-8").strip()
            except Exception:
                setup = ""
    if not setup:
        return None
    try:
        d0 = datetime.date.fromisoformat(setup[:10])
        return 90 - (datetime.date.today() - d0).days
    except Exception:
        return None


def update_archive(out_dir: Path, generated: datetime.datetime, html: str) -> None:
    """
    D13 — maintain a 30-day briefing archive.
    Writes a dated copy of the briefing plus archive.json / archive.html
    into the output folder for the publish step to deploy alongside index.html.
    Prior manifest is fetched back from the published site (fresh CI workspace).
    """
    dated_name = f"briefing_{generated.strftime('%Y-%m-%d')}.html"
    try:
        (out_dir / dated_name).write_text(html, encoding="utf-8")
    except Exception as e:
        print(f"   ⚠️  Archive: could not write dated copy: {e}")
        return

    items = []
    try:
        r = requests.get(f"{BRIEFING_SITE_URL}/archive.json", timeout=10)
        if r.ok:
            items = r.json().get("items", [])
    except Exception:
        pass
    local_manifest = Path(__file__).parent / "archive.json"
    if not items and local_manifest.exists():
        try:
            items = json.loads(local_manifest.read_text(encoding="utf-8")).get("items", [])
        except Exception:
            items = []

    today_iso = generated.strftime("%Y-%m-%d")
    items = [i for i in items if i.get("date") != today_iso]
    items.append({
        "date":  today_iso,
        "label": generated.strftime(f"%A {DAYFMT} %B %Y"),
        "file":  dated_name,
    })
    items = sorted(items, key=lambda i: i["date"], reverse=True)[:30]
    manifest = json.dumps({"items": items}, indent=1)
    try:
        (out_dir / "archive.json").write_text(manifest, encoding="utf-8")
        local_manifest.write_text(manifest, encoding="utf-8")
    except Exception as e:
        print(f"   ⚠️  Archive: could not write manifest: {e}")

    rows = "".join(
        f'<a class="row" href="{i["file"]}"><span class="d">{i["date"]}</span>'
        f'<span class="l">{i["label"]}</span></a>'
        for i in items
    )
    archive_html = f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Briefing Archive</title><style>
body{{font-family:'Source Sans 3','Helvetica Neue',sans-serif;background:#f5f2eb;color:#1a1a1a;
max-width:640px;margin:0 auto;padding:2rem 1rem}}
h1{{font-family:Georgia,serif;font-size:1.4rem;border-bottom:3px solid #c0392b;padding-bottom:.5rem}}
.row{{display:flex;gap:1rem;padding:.6rem .75rem;border-bottom:1px solid #cdc8b5;
text-decoration:none;color:#1a1a1a;font-size:.9rem}}
.row:hover{{background:#fff}}
.d{{font-weight:700;min-width:100px;color:#c0392b}}
.l{{color:#484848}}</style></head><body>
<h1>📚 Morning Briefing — Archive (last 30 days)</h1>{rows}
</body></html>"""
    try:
        (out_dir / "archive.html").write_text(archive_html, encoding="utf-8")
        print(f"   ✓ Archive updated: {len(items)} briefing(s) indexed")
    except Exception as e:
        print(f"   ⚠️  Archive: could not write index: {e}")
...
def main():
    ...
    print("\nMorning Briefing Generator — " + generated.strftime(f"%A {DAYFMT} %B %Y"))
    print("=" * 55)
    ...
    all_topic_stories = {}
    topic_feed_cache = prefetch_topic_feeds() if active_topics else {}
    for topic in active_topics:
        print(f"\n{topic['emoji']}  Topic: {topic['name']}")
        items = fetch_topic_items(topic["keywords"], topic_feed_cache)
    ...
    gsh_gas_data = fetch_wallumbilla_price()
    if gsh_gas_data:
        print(f"   ✓ Wallumbilla GSH: ${gsh_gas_data['price']:.2f} $/GJ")
    else:
        print("   ⚠️  Wallumbilla GSH price unavailable")
    if hh_gas_data:
    ...
    # Gas price history (fetched back from the published site each run)
    ...
    # ── Archive index (D13) ──
    update_archive(out_dir, generated, html)

    # ── Audio edition (D12) — optional, requires edge-tts ──
    audio_path = None
    try:
    ...
    if _OUTLOOK_AVAILABLE and os.environ.get("OUTLOOK_CLIENT_ID") and os.environ.get("BRIEFING_EMAIL_TO"):
        ...
        _outlook.send_briefing_email(html, out_dir.name, generated, all_sections, email_analysis,
                                     extra_attachments=[audio_path] if audio_path else None)
